...
pub async fn start_airdrop_mdns_responder(device_name: String) {
    ...
    let socket = match socket2::Socket::new(
        ...
        Ok(s) => s,
        Err(e) => {
            tracing::warn!("AirDrop mDNS responder: socket creation failed: {e}");
            return;
        }
    };
    ...
    if let Err(e) = socket.bind(&bind_addr.into()) {
        tracing::warn!("AirDrop mDNS responder: port 5353 bind failed ({e}); skipping listener");
        return;
    }

    if let Err(e) = socket.join_multicast_v4(&multicast_addr, &std::net::Ipv4Addr::UNSPECIFIED) {
        tracing::warn!("AirDrop mDNS responder: join multicast failed ({e}); skipping listener");
        return;
    }
    ...
    let tokio_socket = match tokio::net::UdpSocket::from_std(std_socket) {
        Ok(s) => s,
        Err(e) => {
            tracing::warn!("AirDrop mDNS responder: failed to register socket with tokio: {e}");
            return;
        }
    };
