...
export default function SessionBehavior({ config, setConfig }: SessionBehaviorProps) {
  ...
  const AudioCodecPicker = ({
    value,
    onChange,
    disabled,
  }: {
    value: AudioCodec;
    onChange: (v: AudioCodec) => void;
    disabled: boolean;
  }) => {
    ...
    const labelFor = (v: AudioCodec) =>
      v === 'auto'
        ? t('sessionBehavior.audioCodecAuto')
        : v === 'opus'
        ? t('sessionBehavior.audioCodecOpus')
        : v === 'aac'
        ? t('sessionBehavior.audioCodecAac')
        : v === 'flac'
        ? t('sessionBehavior.audioCodecFlac')
              ...
              <div className="absolute right-0 top-full mt-1 z-50 min-w-[88px] bg-zinc-950 border border-zinc-800 rounded-md shadow-2xl py-1 backdrop-blur-xl animate-in fade-in zoom-in-95 duration-100">
                {AUDIO_CODEC_VALUES.map(opt => (
                  <div
                    ...
                    className={`px-2 py-1 text-[9px] uppercase tracking-wider font-bold cursor-pointer transition-colors ${
                      value === opt ? 'bg-primary/20 text-primary' : 'text-zinc-400 hover:bg-primary hover:text-on-primary'
                    }`}
                  >
                    {labelFor(opt)}
                  </div>
  ...
  const Toggle = ({
    ...
    >
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          <span
            className={`text-[10px] font-bold uppercase tracking-tight truncate transition-colors ${
              checked ? (danger ? 'text-red-400' : 'text-zinc-200') : 'text-zinc-500 group-hover:text-zinc-400'
            }`}
          >
            {label}
          </span>
      ...
      <div
        className={`w-6 h-3.5 shrink-0 rounded-full p-0.5 transition-all duration-300 ${
          checked ? (danger ? 'bg-red-600' : 'bg-primary') : 'bg-zinc-800'
        }`}
      >
        <div
          className={`w-2.5 h-2.5 rounded-full shadow-sm transition-all duration-300 ${
            checked ? (danger ? 'bg-white translate-x-2.5' : 'bg-[var(--text-on-primary)] translate-x-2.5') : 'bg-white translate-x-0'
          }`}
        />
      </div>
    </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-tight">
                {t('sessionBehavior.recordPath')}
              </span>
            </div>
            <button onClick={handlePickFolder} className="text-[8px] font-black uppercase text-primary hover:text-white transition-colors">
