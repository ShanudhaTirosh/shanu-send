...
import { Download, FolderOpen, RefreshCcw, Palette, HelpCircle, X, ExternalLink, Sun, Moon, Monitor } from 'lucide-react';
  ...
  return (
    <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-2 py-4">
            ...
            <div className="space-y-4 text-xs leading-relaxed text-zinc-300">
              ...
              <ol className="list-decimal list-inside space-y-3 font-medium">
                ...
                <li>
                  ...
                  <code className="text-primary font-bold">{t('header.manualStep4Executable')}</code> {t('header.manualStep4After')}
                </li>
              </ol>
          ...
          <div className="flex items-center gap-2">
            {[
              ...
              <button
                ...
                className={`w-4 h-4 rounded-full transition-all hover:scale-125 active:scale-95 relative group/swatch ${
                  currentTheme === th.id ? 'ring-2 ring-white ring-offset-2 ring-offset-black scale-110' : 'opacity-50 hover:opacity-100'
                }`}
            ...
            {(
              ...
              <button
                ...
                className={`relative p-1.5 rounded-md transition-all group/btn ${
                  colorMode === id ? 'bg-primary text-on-primary shadow-sm' : 'text-zinc-400 hover:text-primary hover:bg-zinc-800/60'
                }`}
              >
                <Icon size={11} />
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-zinc-600 text-[9px] uppercase tracking-[0.3em] font-black mt-1 ml-0.5">{t('header.tagline')}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center flex-1 justify-end">
        ...
        <div className="glass px-4 py-2 rounded-xl flex items-center gap-4 w-full md:w-auto justify-between md:justify-start border border-zinc-800 bg-zinc-950/50 backdrop-blur-2xl shadow-2xl relative group overflow-hidden">
          ...
          <div className="flex flex-col min-w-[110px] relative z-10">
            <div className="flex items-center gap-1.5 mb-1 text-zinc-500">
              ...
              <div
                className={`w-1.5 h-1.5 rounded-full ${
                  binaryStatus.found
                    ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] animate-[pulse_2s_infinite]'
                    : 'bg-yellow-500 animate-pulse'
                }`}
              />
            </div>
            <div
              className={`text-xs font-black uppercase tracking-tighter truncate max-w-[150px] ${
                binaryStatus.found ? 'text-emerald-400 animate-[pulse_4s_infinite]' : 'text-yellow-500'
              }`}
            >
              {isDownloading
          ...
          <div className="flex gap-2 items-center border-l border-zinc-800 pl-3 relative z-10">
            ...
            <button onClick={onSetPath} className="p-1 hover:text-primary text-zinc-500 transition-colors" title={t('header.selectFolder')}>
              <FolderOpen size={16} />
            </button>
            <button onClick={onResetPath} className="p-1 hover:text-red-400 text-zinc-500 transition-colors" title={t('header.resetPath')}>
              <RefreshCcw size={16} />
            </button>
          </div>
        </div>
