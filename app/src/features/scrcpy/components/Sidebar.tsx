    ...
    <>
          ...
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 border-b border-zinc-800/50 pb-2 mb-1">
            ...
            <div className="flex gap-2 items-center ml-auto">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 bg-zinc-800/50 hover:bg-primary/20 border border-zinc-800 hover:border-primary/30 rounded-md text-[9px] font-black uppercase tracking-widest text-primary hover:text-white transition-all group/refresh ${
                  isRefreshing ? 'opacity-50 cursor-not-allowed' : ''
                }`}
              >
                <RefreshCw
                  size={10}
                  className={`group-hover/refresh:rotate-180 transition-transform duration-500 ${
          ...
          <div className="space-y-2">
                    ...
                    >
                      <div
                        className={`p-1.5 rounded-md transition-colors ${
                          isSelected ? 'bg-primary text-on-primary' : 'bg-zinc-800 text-zinc-500 group-hover:text-zinc-300'
                        }`}
                      >
                        <Smartphone size={14} />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        >
                          {deviceFriendlyNames[d] || deviceModels[d] || formatDeviceLabel(d)}
                        </p>
            ...
            <div className="bg-black/40 p-1 rounded-lg flex gap-1 border border-zinc-800/50">
              <button
                ...
                className={`flex-1 py-1.5 text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 rounded-md transition-all ${
                  activeTab === 'usb'
                    ? 'bg-primary text-on-primary shadow-lg translate-y-[-1px]'
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={isRefreshing}
                      className={`px-4 bg-zinc-800 hover:bg-primary text-zinc-400 hover:text-on-primary rounded-lg text-[10px] font-black uppercase transition-all active:scale-95 ${
                        isRefreshing ? 'opacity-50 cursor-not-allowed' : ''
                      }`}
                    >
                        ...
                        .map((dev, idx) => (
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <span
                                className="text-[10px] font-bold text-zinc-400 group-hover:text-zinc-200 truncate"
                                title={`${dev.address} (${formatMdnsName(dev.name)})`}
                              >
                ...
                <div className="space-y-3 pt-1">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input
                      ...
                      className={`w-full bg-black/40 border rounded-lg px-2.5 py-1.5 text-xs text-zinc-200 transition-all outline-none ${
                        pairIp ? 'border-amber-400/50 focus:border-amber-400' : 'border-zinc-800 focus:border-primary/40'
                      }`}
                    />
                  </div>
                  <button
                    ...
                    disabled={isRefreshing}
                    className={`w-full py-1.5 border border-zinc-800 hover:border-primary/50 hover:bg-primary/5 text-zinc-500 hover:text-primary rounded-lg text-[10px] font-black uppercase transition-all active:scale-[0.98] ${
                      isRefreshing ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
      ...
      {pairingTarget && (
        <div
          ...
          onClick={e => {
            if (e.target === e.currentTarget) setPairingTarget(null);
          }}
        >
          ...
          <div className="relative w-full max-w-sm bg-zinc-950/90 border border-primary/30 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] overflow-hidden backdrop-blur-2xl p-6">
            ...
            {pairingTarget.service.includes('_adb-tls-pairing') ? (
              <>
                ...
                <p className="text-[11px] text-zinc-500 mb-4 truncate">
                  {pairingTarget.address} ({formatMdnsName(pairingTarget.name)})
                </p>
                <input
                  autoFocus
                  type="text"
                  ...
                  onKeyDown={e => {
                    if (e.key === 'Enter') submitPairingModal();
                  }}
              ...
              <>
                <h3 className="text-base font-black uppercase tracking-tight text-white mb-1 pr-6">
                  {t('sidebar.pairModalNotReadyTitle')}
                </h3>
                <p className="text-[11px] text-zinc-500 mb-4 truncate">
                  {pairingTarget.address} ({formatMdnsName(pairingTarget.name)})
                </p>
