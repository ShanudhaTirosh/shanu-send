...
export function buildRendererOptions(renderDriverSupport: RenderDriverSupport, autoLabel: string = 'Auto'): RendererOption[] {
  return [
    ...
    return (
      <div className={`relative ${className}`} ref={containerRef}>
        ...
        >
          ...
          <ChevronDown
            size={14}
            className={`text-zinc-500 group-hover:text-primary transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          />
        </button>
          ...
          <div className="absolute top-full left-0 right-0 mt-1 bg-zinc-950 border border-zinc-800 rounded-md shadow-2xl z-[100] py-1 overflow-hidden animate-in fade-in zoom-in-95 duration-100 backdrop-blur-xl">
            {options.map(opt => (
              <div
                ...
                className={`px-2 py-1.5 text-[11px] cursor-pointer transition-colors ${
                  opt.value === value ? 'bg-primary/20 text-primary font-bold' : 'text-zinc-400 hover:bg-primary hover:text-on-primary font-medium'
                }`}
              >
                {opt.label}
              </div>
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${
                config.vsync !== false ? 'bg-primary border-primary' : 'border-zinc-700 group-hover:border-primary'
              }`}
            >
              {config.vsync !== false && <div className="w-1.5 h-1.5 bg-black rounded-[1px]" />}
            </div>
            <span className="text-[10px] font-bold uppercase text-zinc-300 tracking-wide group-hover:text-primary transition-colors">
              {t('controlPanel.vsync')}
            </span>
          </div>
          <span className="text-[8px] font-black uppercase tracking-tighter text-zinc-600 group-hover:text-primary/70 transition-colors">
            {t('controlPanel.vsyncHint')}
          </span>
        </div>
      </Tooltip>
      <Tooltip text={t('controlPanel.ignoreEncoderConstraintsTooltip')}>
        ...
        >
          <div className="flex items-center gap-2">
            <div
              className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${
                config.ignoreVideoEncoderConstraints ? 'bg-primary border-primary' : 'border-zinc-700 group-hover:border-primary'
              }`}
            >
              {config.ignoreVideoEncoderConstraints && <div className="w-1.5 h-1.5 bg-black rounded-[1px]" />}
            </div>
            <span className="text-[10px] font-bold uppercase text-zinc-300 tracking-wide group-hover:text-primary transition-colors">
              {t('controlPanel.ignoreEncoderConstraints')}
            </span>
          </div>
          <span className="text-[8px] font-black uppercase tracking-tighter text-zinc-600 group-hover:text-primary/70 transition-colors">
            v4.1
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1.5 py-2.5 rounded-lg transition-all ${
              config.sessionMode === 'mirror'
                ...
                : 'text-zinc-500 hover:text-primary hover:bg-zinc-950 transition-all'
            }`}
          >
        ...
        <div className="space-y-2.5 relative z-30">
          {config.sessionMode === 'mirror' && (
            <>
              <div className="space-y-3 p-3 rounded-xl border border-zinc-800 bg-zinc-950/20">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-[9px] font-black uppercase text-zinc-400 tracking-widest">
                    {t('controlPanel.inputEnhancements')}
                  </span>
                </div>
                <p className="text-[8px] text-zinc-500 leading-relaxed mb-1">{t('controlPanel.inputEnhancementsDescription')}</p>
                ...
                <div className="grid grid-cols-1 gap-3">
                    ...
                    <div className="flex items-start gap-3 cursor-pointer group" onClick={() => handleChange('hidKeyboard', !config.hidKeyboard)}>
                      <div
                        className={`mt-0.5 w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${
                          config.hidKeyboard ? 'bg-primary border-primary' : 'border-zinc-700 group-hover:border-primary'
                        }`}
                      >
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] font-bold uppercase text-zinc-300 tracking-wide group-hover:text-primary">
                            {t('controlPanel.hidKeyboard')}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <Tooltip text={t('controlPanel.hidMouseTooltip')}>
                    <div className="flex items-start gap-3 cursor-pointer group" onClick={() => handleChange('hidMouse', !config.hidMouse)}>
                      <div
                        className={`mt-0.5 w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${
                          config.hidMouse ? 'bg-primary border-primary' : 'border-zinc-700 group-hover:border-primary'
                        }`}
                      >
                  ...
                  {(config.hidKeyboard || config.hidMouse) && (
                    <Tooltip text={t('controlPanel.pureHidTooltip')}>
                      <div
                        className="flex items-start gap-3 ml-0.5 cursor-pointer group animate-in slide-in-from-top-1 duration-200"
                        onClick={() => handleChange('otgPure', !config.otgPure)}
                      >
                        <div
                          className={`mt-0.5 w-3 h-3 rounded border flex items-center justify-center transition-colors ${
                            config.otgPure ? 'bg-red-500 border-red-500' : 'border-zinc-700 group-hover:border-red-500'
                          }`}
                        >
                        ...
                        <div className="flex flex-col">
                          <span
                            className={`text-[9px] font-bold uppercase tracking-wider transition-colors ${
                              config.otgPure ? 'text-red-400' : 'text-zinc-500 group-hover:text-red-400'
                            }`}
                          >
                            {t('controlPanel.pureHid')}
                          </span>
                        </div>
                      </div>
                    </Tooltip>
              ...
              <div
                className={`space-y-2.5 pt-0.5 transition-all duration-300 ${
                  config.otgPure ? 'opacity-30 pointer-events-none grayscale' : 'opacity-100'
                }`}
              >
                <PerformanceGrid />
          ...
          {config.sessionMode === 'camera' && (
            <div className="space-y-3 animate-in fade-in duration-300">
              <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 flex gap-3 group/tip hover:bg-primary/10 transition-all">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-zinc-400 leading-relaxed">
                    {t('controlPanel.webcamProTipTextBefore')}{' '}
                    <span className="text-zinc-200 font-bold">{t('controlPanel.webcamProTipObs')}</span>{' '}
                    {t('controlPanel.webcamProTipAndStart')}{' '}
                    <span className="text-zinc-200 font-bold">{t('controlPanel.webcamProTipVirtualCamera')}</span>
                    {t('controlPanel.webcamProTipTextAfter')}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center h-4">
                  <div className="flex items-center gap-1.5">
                    <label className="text-[9px] font-black text-zinc-500 uppercase tracking-tighter">
                      {t('controlPanel.cameraDevice')}
                    </label>
                ...
                <CustomSelect
                  ...
                  options={[{ value: '', label: t('controlPanel.autoSelect') }, ...detectedCameras.map(cam => ({ value: cam.id, label: cam.name }))]}
                />
              </div>
              ...
              <div className="space-y-2 p-2.5 rounded-xl border border-zinc-800/60 bg-zinc-950/20">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-[7px] font-bold px-1 py-0.5 rounded bg-primary/10 text-primary border border-primary/20 uppercase tracking-widest">
                    {t('controlPanel.badgeNew')}
                  </span>
                </div>
                <Tooltip text={t('controlPanel.cameraTorchTooltip')}>
                  <div
                    className="flex items-center justify-between gap-2 cursor-pointer group"
                    onClick={() => handleChange('cameraTorch', !config.cameraTorch)}
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${
                          config.cameraTorch ? 'bg-primary border-primary' : 'border-zinc-700 group-hover:border-primary'
                        }`}
                      >
                        {config.cameraTorch && <div className="w-1.5 h-1.5 bg-black rounded-[1px]" />}
                      </div>
                      <span className="text-[10px] font-bold uppercase text-zinc-300 tracking-wide group-hover:text-primary transition-colors">
                        {t('controlPanel.cameraTorch')}
                      </span>
                    </div>
                  </div>
                </Tooltip>
              ...
              <div className="p-3.5 rounded-xl border border-zinc-800 bg-zinc-950/40 space-y-4">
                <div className="flex items-center justify-between border-b border-zinc-800/50 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[10px] font-black uppercase text-zinc-300 tracking-widest">
                      {t('controlPanel.virtualDisplayEngine')}
                    </h3>
                  </div>
                  <div className="flex items-center gap-3">
                    <Tooltip text={t('controlPanel.flexDisplayTooltip')}>
                      <button
                        ...
                        className={`flex items-center gap-1.5 transition-colors ${
                          config.flexDisplay ? 'text-primary' : 'text-zinc-600 hover:text-zinc-400'
                        }`}
                        title={t('controlPanel.flexDisplay')}
                      >
                        <span
                          className={`text-[8px] font-black uppercase tracking-tighter px-1 py-0.5 rounded border transition-colors ${
                            config.flexDisplay ? 'bg-primary/10 border-primary/40 text-primary' : 'border-zinc-700 text-zinc-600'
                          }`}
                        >
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 transition-colors ${
                        config.aspectRatioLock ? 'text-primary' : 'text-zinc-600 hover:text-zinc-400'
                      }`}
            ...
            <span className="relative z-10 flex items-center justify-center gap-3 text-on-primary">
              ...
              {config.sessionMode === 'mirror'
                ? (config.hidKeyboard || config.hidMouse) && config.otgPure
                  ? t('controlPanel.initializeOtg')
                  : t('controlPanel.startMission')
                : config.sessionMode === 'camera'
                ? t('controlPanel.engageCamera')
                : t('controlPanel.ejectToDesktop')}
            </span>
          </button>
        ) : (
          <button
